...
var processDirectory = require('./processDirectory'),
    ...
    suffixes = ['/index.md', '/index.markdown', '', '.md', '.markdown', 'txt'];
...
module.exports = function(rootDir) {
    ...
    return function(req, res) {
        ...
        function checkDone() {
            ...
            var allError = processed.every(function(r) {
                return r && r.err;
            });
        ...
        // handle space of the file name and so on
        reqPath = decodeURI(reqPath);
        if (fs.existsSync(reqPath)) {
            var stat = fs.statSync(reqPath);
            if (stat.isFile()) {
                return processFile(reqPath, res);
            } 
            return processDirectory(reqPath, res);
        } else {
            res.writeHead(404, { 'content-type': 'text/plain' });
            res.end('not found');
            return;
