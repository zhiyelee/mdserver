            ...
            function check() {
                ...
                var body = results.map(function(res) {
                    ...
                    return '<li class="file-list__item">' +
                                '<span class="item__type">[' + cls + ']</span> ' +
                                '<a href="' + win(res.file) + '">' + filename + '</a>'+
                            '</li>';
                ...
                body = '<ul class="file-list">' + body + '</ul>';

                var title = win(dirPath.substring(rootDir.length) || '/');

                var rendered = simpleMustache(template, {
                    title: title,
                    body: '<div class="pg-file-list"><h1> ls of dir: ' + title  + '</h1>' + body + '</div>',
